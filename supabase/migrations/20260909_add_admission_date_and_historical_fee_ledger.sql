...
set admission_date = coalesce(approved_at::date, created_at::date, current_date)
...
begin
  ...
  if v_admission_date is null then return; end if;

  v_month := date_trunc('month', v_admission_date)::date;
...
comment on column public.admissions.admission_date is 'Actual student admission date; used as the start of the monthly fee ledger.';
