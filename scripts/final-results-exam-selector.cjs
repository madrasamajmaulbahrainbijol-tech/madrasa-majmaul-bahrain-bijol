...
// Shared marksheet: replace the static Academic Record badge with the actual result.
{
...
if(s.includes(oldPrint)){s=s.replace(oldPrint,newPrint);fs.writeFileSync(results,s)}
...
if(s.includes(oldLoad)){s=s.replace(oldLoad,newLoad);fs.writeFileSync(results,s)}
...
if(s.includes(stateNeedle)){s=s.replace(stateNeedle,stateReplacement);fs.writeFileSync(results,s)}
...
if(s.includes(latestNeedle)){s=s.replace(latestNeedle,latestReplacement);fs.writeFileSync(results,s)}
...
if(s.includes(pairsNeedle)){s=s.replace(pairsNeedle,pairsReplacement);fs.writeFileSync(results,s)}
...
if(s.includes(controlStart)){replaceBetween(results,controlStart,controlEnd,controlReplacement,"exam selector controls");}
