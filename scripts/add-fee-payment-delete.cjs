...
function patchProfile() {
  ...
  const reloadSelect = 'select("id,fee_month,amount_due,amount_paid,payment_date,payment_method,receipt_number,remarks")';
  s = s.replaceAll(reloadSelect, feeSelectAudit);
  ...
  const handler = ' const deleteFeePayment=async(historyId:string)=>{if(!isSuperAdmin)return;if(!window.confirm("Delete this payment history record? The audit record will be kept and marked deleted."))return;setNotice("");setError("");const {error:e}=await supabase.rpc("delete_student_fee_payment",{p_history_id:historyId});if(e){setError(e.message||"Unable to delete fee payment record.");return}setNotice("Fee payment history deleted by Super Admin.");await load()};\n';
