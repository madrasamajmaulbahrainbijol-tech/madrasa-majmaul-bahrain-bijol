...
function patchProfile() {
  ...
  if (!s.includes('import FeePaymentHistory from "../../../../components/admin/FeePaymentHistory";')) s = s.replace(importMarker, importMarker + '\nimport FeePaymentHistory from "../../../../components/admin/FeePaymentHistory";');
  ...
  if (!s.includes('const [paymentHistory,setPaymentHistory]')) s = s.slice(0, stateIndex) + ' const [paymentHistory,setPaymentHistory]=useState<any[]>([]),[isSuperAdmin,setIsSuperAdmin]=useState(false);\n' + s.slice(stateIndex);
