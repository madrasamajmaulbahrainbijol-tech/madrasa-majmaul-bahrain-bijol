...
export default function StudentDashboard() {
  ...
  const feeSummary = useMemo(() => {
    const due = fees.reduce((sum, row) => sum + Number(row.amount_due || 0), 0);
  ...
  return (
    <main className="min-h-screen bg-[#f4f7f6] text-slate-900">
      ...
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        ...
        <nav className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5 print:hidden">
          <a href="#profile" className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black shadow-sm ring-1 ring-slate-100 hover:bg-green-50">Profile</a>
          <a href="#documents" className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black shadow-sm ring-1 ring-slate-100 hover:bg-green-50">Documents</a>
          <a href="#attendance" className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black shadow-sm ring-1 ring-slate-100 hover:bg-green-50">Attendance</a>
          <a href="#results" className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black shadow-sm ring-1 ring-slate-100 hover:bg-green-50">Results</a>
          <a href="#fees" className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black shadow-sm ring-1 ring-slate-100 hover:bg-green-50">Fees</a>
        </nav>

        <section className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-4 print:hidden"><div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><p className="text-xs font-black uppercase tracking-[1.5px] text-slate-500">Attendance</p><p className="mt-2 text-3xl font-black text-green-800">{attendanceSummary.percentage}%</p><p className="mt-1 text-sm text-slate-500">{attendanceSummary.present} present / {attendanceSummary.total} total</p></div><div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><p className="text-xs font-black uppercase tracking-[1.5px] text-slate-500">Examinations</p><p className="mt-2 text-3xl font-black">{exams.length}</p><p className="mt-1 text-sm text-slate-500">Official exam records</p></div><div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><p className="text-xs font-black uppercase tracking-[1.5px] text-slate-500">Fee Balance</p><p className="mt-2 text-3xl font-black text-green-800">₹{feeSummary.balance.toLocaleString("en-IN")}</p><p className="mt-1 text-sm text-slate-500">₹{feeSummary.paid.toLocaleString("en-IN")} paid of ₹{feeSummary.due.toLocaleString("en-IN")}</p></div><div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><p className="text-xs font-black uppercase tracking-[1.5px] text-slate-500">Account</p><p className="mt-2 text-lg font-black text-green-800">Verified</p><p className="mt-1 text-sm text-slate-500">Read-only official profile</p></div></section>

        <section id="profile" className="mt-7 scroll-mt-24 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
        ...
        <section id="documents" className="mt-7 scroll-mt-24 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
        ...
        <section id="attendance" className="mt-7 scroll-mt-24 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
        ...
        <section id="results" className="mt-7 scroll-mt-24 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
          <div className="mb-6"><p className="text-xs font-black uppercase tracking-[2px] text-green-700">04</p><h2 className="mt-1 text-2xl font-black">Examinations & Results</h2><p className="mt-1 text-sm text-slate-500">Official results published by the Madrasa office.</p></div>
          {exams.length ? <div className="space-y-5">{exams.map((exam) => { const examMarks = marks.filter((m) => m.exam_id === exam.id); return <div key={exam.id} className="rounded-2xl border border-slate-200 p-5"><div className="flex flex-col justify-between gap-2 sm:flex-row"><div><h3 className="font-black text-slate-900">{exam.exam_name}</h3><p className="mt-1 text-sm text-slate-500">{exam.exam_type} • {formatDate(exam.exam_date)} {exam.session ? `• ${exam.session}` : ""}</p></div>{examMarks.length ? <span className="w-fit rounded-full bg-green-100 px-3 py-1 text-xs font-black text-green-800">Result Published</span> : null}</div>{examMarks.length ? <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[650px] text-left text-sm"><thead><tr className="border-b bg-slate-50"><th className="px-3 py-2">Subject</th><th className="px-3 py-2">Max</th><th className="px-3 py-2">Obtained</th><th className="px-3 py-2">Grade</th><th className="px-3 py-2">Result</th></tr></thead><tbody>{examMarks.map((m, i) => <tr key={`${exam.id}-${i}`} className="border-b"><td className="px-3 py-2 font-bold">{m.subject}</td><td className="px-3 py-2">{m.max_marks}</td><td className="px-3 py-2 font-black">{m.obtained_marks}</td><td className="px-3 py-2">{m.grade || "—"}</td><td className="px-3 py-2 font-black">{m.result_status}</td></tr>)}</tbody></table></div> : <p className="mt-4 text-sm text-slate-500">Marks have not been published for this examination yet.</p>}</div>; })}</div> : <p className="text-slate-500">No examination records have been published yet.</p>}
        </section>

        <section id="fees" className="mt-7 scroll-mt-24 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
          <div className="mb-6"><p className="text-xs font-black uppercase tracking-[2px] text-green-700">05</p><h2 className="mt-1 text-2xl font-black">Fee Record</h2><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">Total Due</p><p className="mt-1 text-xl font-black">₹{feeSummary.due.toLocaleString("en-IN")}</p></div><div className="rounded-xl bg-green-50 p-4"><p className="text-xs font-black uppercase text-green-700">Total Paid</p><p className="mt-1 text-xl font-black text-green-800">₹{feeSummary.paid.toLocaleString("en-IN")}</p></div><div className="rounded-xl bg-amber-50 p-4"><p className="text-xs font-black uppercase text-amber-700">Balance</p><p className="mt-1 text-xl font-black text-amber-900">₹{feeSummary.balance.toLocaleString("en-IN")}</p></div></div></div>
